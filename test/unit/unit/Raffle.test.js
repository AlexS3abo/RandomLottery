    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("checkUpkeep", function () {
              it("returns false if people havent sent any ETH", async function () {
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  const { upkeepNeeded } = await raffle.callStatic.checkUpkeep([]) //getting the static call not to kick off the transaction
                  assert(!upkeepNeeded)
              })
              it("returns false if raffle isnt open", async function () {
                  ...
                  const raffleState = await raffle.getRaffleState() //getting the static call not to kick off the transaction
                  const { upkeepNeeded } = await raffle.callStatic.checkUpkeep([]) //getting the static call not to kick off the transaction
